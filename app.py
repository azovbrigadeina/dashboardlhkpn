...
st.markdown("""
    ...
    /* Card Metric Putih dengan Border Halus */
    ...
    /* Header & Footer Streamlit disembunyikan */
    header {visibility: hidden;}
    footer {visibility: hidden;}
    </style>
""", unsafe_allow_html=True)

# --- 3. DATA LOGIC (Sama seperti sebelumnya) ---
def clean_data(df, filter_bulan):
    df.columns = df.columns.str.strip()
    df['NIK_KEY'] = df['NIK'].astype(str).str.replace("'", "")
...
# --- 4. AUTHENTICATION (PURE LIGHT LOGIN) ---
...
with st.sidebar:
    ...
    if st.button("🚪 Keluar"):
        ...
        st.rerun()
    st.divider()
    file_in = st.file_uploader("Upload Data Excel", type=["xlsx"])

if file_in:
    raw = pd.read_excel(file_in)
    bln_list = ["GLOBAL (AKUMULASI)"] + sorted([str(b).upper() for b in raw['BULAN'].unique() if pd.notna(b)])
    sel_bln = st.sidebar.selectbox("Filter Periode:", bln_list)
    ...
    st.markdown(f"<p style='color: #667085; font-size: 16px; margin-top: -15px;'>Universitas Jambi — Periode: {sel_bln}</p>", unsafe_allow_html=True)
    st.write("#")
    ...
    with c_pie:
        ...
        fig_p.update_layout(showlegend=True, margin=dict(t=0,b=0,l=0,r=0))
        st.plotly_chart(fig_p, use_container_width=True)

    with c_bar:
        st.markdown("##### Unit Kerja dengan Belum Lapor Terbanyak")
